...
이 디렉토리는 **모델과 진입점**만 가진다 (양자화 코드는 `dvslib/quant`). 최종 목표는 FPGA 배포 (MobileOne S0 + PT2E QAT → INT8).
...
├── train_qat.py           # PT2E QAT fine-tune 진입점 (dvslib.quant.pt2e 사용): FP32 ckpt → reparam → QAT → INT8 저장
├── model.py               # 모델 정의 (mobilenet_v2, mobileone_s0). `python model.py`로 self-test
├── mobileone_official.py  # Apple 공식 구현 — 수정 금지
├── model_summary.py       # 구조·파라미터·메모리 분석 (reparam 전/후, --int8 PT2E 복원 검증)
├── export_mobileone_info.py  # FPGA용 weight·구조 추출 — **eager 기반이라 현재 동작 안 함**, PT2E 재작성 예정 (Phase 2)
...
dvslib에서 오는 것: `data`(bin I/O·Dataset·blocked split), `eval`(metrics·evaluate), `training`(Trainer·callbacks·seed), `tracking`(wandb), `quant`(PT2E QAT: prepare_qat/convert/set_qat_mode/save_int8/load_int8).
...
QAT 산출물: `runs/qat_mobileone_s0_pretrained_pt2e/` (`*_int8.pth` = prepared state_dict, INT8 7.12px — `BASELINE.md`).
...
- QAT는 **PT2E(`torch.export`) 기반 `dvslib/quant/pt2e.py`** 만 사용 (모든 방식 공용). eager QAT는 폐기됐으므로 되살리지 말 것. PT2E 그래프는 `.train()/.eval()`을 못 쓰므로 dvslib 루프에 `set_qat_mode` 훅을 주입해 재사용한다. FPGA 제약(대칭 INT8)은 `get_fpga_quantizer` 한 곳에만 기술. INT8 저장/복원은 `save_int8`/`load_int8` — prepared(fake-quant) state_dict를 저장하고 복원 시 재prepare·convert (convert된 그래프의 단일 파일 저장은 torch 2.5에서 불가).
...
- 향후 작업: `export_mobileone_info.py` PT2E 그래프 대응 (루트 `research_plan.md` Phase 2).
