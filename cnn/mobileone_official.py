...
class MobileOneBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if inference_mode:
            self.reparam_conv = nn.Conv2d(in_channels=in_channels,
                                          ...
                                          groups=groups,
                                          bias=True)
        else:   # train mode (3 branches)
            # 1. Re-parameterizable skip connection
            self.rbr_skip = nn.BatchNorm2d(num_features=in_channels) \
                if out_channels == in_channels and stride == 1 else None

            # 2. Re-parameterizable conv branches (k duplicated)
            rbr_conv = list()
            ...
            # 3. Re-parameterizable scale branch (1x1 conv)
            self.rbr_scale = None
            if kernel_size > 1: # Only for DepthwiseConv blocks
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 1. Skip branch output
        identity_out = 0
        if self.rbr_skip is not None:
            identity_out = self.rbr_skip(x)

        # 2. Scale branch output (1x1 conv)
        scale_out = 0
        if self.rbr_scale is not None:
            scale_out = self.rbr_scale(x)

        # 3. Other branches (Main k conv branches)
    ...
    def reparameterize(self):
        ...
        if self.inference_mode:
            return
        # Fuse all branches into single branch
        kernel, bias = self._get_kernel_bias()
        # Create a new conv layer for the fused branch
        self.reparam_conv = nn.Conv2d(in_channels=self.rbr_conv[0].conv.in_channels,
                                      ...
                                      bias=True)
        # Copy weights and bias from the fused branch
    ...
    def _get_kernel_bias(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        # 1. Get weights and bias of "scale branch"
        kernel_scale = 0
        bias_scale = 0
        if self.rbr_scale is not None:  # Only for DepthwiseConv blocks
            kernel_scale, bias_scale = self._fuse_bn_tensor(self.rbr_scale)
            # Pad scale branch kernel to match conv branch (1x1 & 3x3) kernel size. 
            pad = self.kernel_size // 2
            kernel_scale = torch.nn.functional.pad(kernel_scale,
                                                   [pad, pad, pad, pad])

        # 2. Get weights and bias of "skip branch"
        kernel_identity = 0
        bias_identity = 0
        if self.rbr_skip is not None:
            kernel_identity, bias_identity = self._fuse_bn_tensor(self.rbr_skip)

        # 3. Get weights and bias of conv branches
        kernel_conv = 0
        bias_conv = 0
        for ix in range(self.num_conv_branches):
    ...
    def _fuse_bn_tensor(self, branch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if isinstance(branch, nn.Sequential):
            # Case 1: Conv-BN block
            kernel = branch.conv.weight             # W (weights)
            running_mean = branch.bn.running_mean   # μ (mean)
            running_var = branch.bn.running_var     # σ^2 (variance)
            gamma = branch.bn.weight                # γ (scale)
            beta = branch.bn.bias                   # β (shift)
            eps = branch.bn.eps                     # ε (avoid division by zero)
        else:
            # Case 2: Only BN block (skip branch)
            assert isinstance(branch, nn.BatchNorm2d)
            # Create Identity kernel (make conv layer as identity)
            if not hasattr(self, 'id_tensor'):
                input_dim = self.in_channels // self.groups
                # Create kernel with 0
                ...
                # Fill center of kernel with 1 (Dirac delta function)
        ...
        # Re-parameterization logic:
        std = (running_var + eps).sqrt()        # Standard deviation
        t = (gamma / std).reshape(-1, 1, 1, 1)    # Scale factor
        
        # Final Weight = W * (gamma / std)
        # Final Bias = beta - (mean * gamma / std)
    ...
    def _conv_bn(self,
        ...
        mod_list.add_module('conv', nn.Conv2d(in_channels=self.in_channels,
                                              ...
                                              stride=self.stride,
                                              padding=padding,
                                              groups=self.groups,   # DepthwiseConv 
                                              bias=False))          # Add bias manually
...
class MobileOne(nn.Module):
    ...
    def __init__(self,
        ...
        # width_multipliers: # of channels in each stage (unit: 64)
        assert len(width_multipliers) == 4, "width_multipliers must be a list of 4 elements"    
        self.inference_mode = inference_mode
        ...
        # Build stages
        # State 0: Input layer (downsampling by stride=2)
    ...
    def _make_stage(self,
        ...
        for ix, stride in enumerate(strides):
            ...
            # Use SE-ReLU for last few blocks
            ...
            blocks.append(MobileOneBlock(in_channels=self.in_planes,
                                         ...
                                         use_se=use_se,
                                         num_conv_branches=self.num_conv_branches))
            self.in_planes = planes    # Update input channels for next stage
            self.cur_layer_idx += 1
...
PARAMS = {
    # width multipliers: (stage 0, stage 1, stage 2, stage 3)
