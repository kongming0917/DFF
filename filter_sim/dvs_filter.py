...
class BinProcessor:
    ...
    def read_frames(self, bin_file_path: str, max_frames: Optional[int] = None) -> List[DVSFrame]:
        ...
        print(f"Read {len(frames)} frames from {bin_file_path}")
        return frames
    ...
    def process_file(self, input_path: str, output_path: Optional[str] = None) -> List[DVSFrame]:
        ...
        self.stats.events_before = total_events_before
        self.stats.events_after = total_events_after
    ...
    def print_stats(self):
        ...
        print(f"Frames filtered out:   {self.stats.filtered_frames}")
        print(f"Processing time:       {self.stats.processing_time_sec:.3f} sec")
        print(f"Events before:         {self.stats.events_before}")
        print(f"Events after:          {self.stats.events_after}")
        if self.stats.events_before > 0:
            reduction = (1 - self.stats.events_after / self.stats.events_before) * 100
...
class KalmanPointExtractor(CenterPointExtractor):
    ...
    """
    def __init__(self, process_noise: float = 0.5, measurement_noise: float = 10.0):
        super().__init__()
        # filterpy 라이브러리 필요: pip install filterpy
        from filterpy.kalman import KalmanFilter
        from filterpy.common import Q_discrete_white_noise

        # State vector: [x, y] - position only (stationary laser)
        self.kf = KalmanFilter(dim_x=2, dim_z=2)
        self.kf.x = np.array([0., 0.])  # Initial state [x, y]
        
        # State transition matrix (no motion, position remains same)
        ...
        # Process noise covariance (small random drift for stationary target)
