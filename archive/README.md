...
활성 코드는 brownian motion 데이터용 후속 버전(`cnn`, `yolo`, `filter`)과 공통 패키지 `dvslib/`를 사용한다.

> 참고: 이 디렉토리의 코드는 삭제된 `lib/bin_processor.py` shim을 import하므로 그대로는 실행되지 않는다. 참조용으로만 보존.
