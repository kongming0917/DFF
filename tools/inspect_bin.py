#!/usr/bin/env python3
"""
DVS .bin 파일 구조 확인 스크립트 — 헤더·2bit 패킹·이벤트 통계를 프레임 단위로 출력.

  python tools/inspect_bin.py                       # data/gaussian_large.bin, 720x960
  python tools/inspect_bin.py data/gaussian_brownian_512x512.bin --height 512 --width 512
"""
import struct
import numpy as np
import os
...
def analyze_bin_file(bin_file, num_frames=3, frame_width=960, frame_height=720):
    """
    DVS bin 파일을 분석하는 함수
...
def extract_headers_to_txt(bin_file, output_txt=None, max_frames=None, frame_width=960, frame_height=720):
    """
...
def main():
    ...
    ap = argparse.ArgumentParser(description="DVS .bin 구조 분석")
    ap.add_argument("bin_file", nargs="?", default=os.path.join(root, "data", "gaussian_large.bin"))
    ap.add_argument("--num-frames", type=int, default=3, help="상세 분석할 프레임 수")
    ap.add_argument("--height", type=int, default=720)
    ap.add_argument("--width", type=int, default=960)
    ap.add_argument("--extract-headers", action="store_true", help="모든 프레임 헤더를 .txt로 추출")
    ap.add_argument("--max-extract-frames", type=int, default=None)
    args = ap.parse_args()

    if not os.path.exists(args.bin_file):
        print(f"❌ 오류: 파일을 찾을 수 없습니다: {args.bin_file}")
        return 1
    if args.extract_headers:
        extract_headers_to_txt(args.bin_file, max_frames=args.max_extract_frames,
                               frame_width=args.width, frame_height=args.height)
        print()
    analyze_bin_file(args.bin_file, args.num_frames, frame_width=args.width, frame_height=args.height)
    return 0
